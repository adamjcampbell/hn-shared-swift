...
/// call `appModel.dispatch(.toggleRead(id: ...))` directly — iOS as a
/// SwiftUI Button action, Android from a Composable launching a
/// coroutine on the bridged `suspend fun dispatch`.
///
...
/// (`state.searchQuery = it`). `AppModel`'s `runSearchQueryWatcher`
/// observes `state.searchQuery` and fires the debounced fetch
